...
public final class SubtitleDialog extends BaseDialog {
    ...
    protected void initView() {
        ...
        int savedPadding = Setting.getSubtitleBottomPadding();
        bottomPaddingFraction = savedPadding / 1000.0f;
        subtitleView.setBottomPaddingFraction(bottomPaddingFraction);

        int savedTextSize = Setting.getSubtitleTextSize();
        textSize = savedTextSize / 1000.0f;
        subtitleView.setTextSize(androidx.media3.common.util.Util.TEXT_SIZE_UNIT_EM, textSize);
    }
    ...
    private void onSmall(View view) {
        textSize -= TEXT_SIZE_INCREMENT;
        if (textSize < 0.5f) textSize = 0.5f;
        subtitleView.setTextSize(androidx.media3.common.util.Util.TEXT_SIZE_UNIT_EM, textSize);
        Setting.putSubtitleTextSize((int) (textSize * 1000));
    }
